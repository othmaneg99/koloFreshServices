...
// new partner
  ...
  router.post("/", async function (req,res,next) {
    let user = new User({})
    let userData
    if(!req.body.userName) {res.status(500).send("MERCI DE REMPLIR TOUS LES CHAMPS.")}
    else{
      let filters 
    ...
    if(userData.length == 0){
      ...
      for (let i = 0; i < userData.length; i++) {
        ...
        if(hasRole.length !=0) 
        {
            ...
            let userUpdate = new User({password : hashedPassword, isReseted : true})
            await userUpdate.update({_id : userData[i]._id})
            existUser = await user.get({_id : userData[i]._id, isRemoved : false})
            if(existUser[0].email){
                // envoi mail
                const request = new Request()
                const resultEnvoi = await request.postEmail(process.env.EmailService + '/newPassword',   { email : existUser[0].email,nom:   existUser[0].firstName+" "+existUser[0].lastName, password : password} )
