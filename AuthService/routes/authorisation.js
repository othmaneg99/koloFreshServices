    ...
    if(req.body._id == 0) res.status(401).send("TOKEN INVALID")
        ...
        if(req.body.idPartner){
            ...
            if(existUser.length == 0){
                ...
                let role = new Role({})
                let existRole = await role.get({idUser : req.body._id,role : 'admin', isRemoved : false})
                if(existRole){
                    let existPartner = await role.get({idUser : req.body.idPartner, role: 'partner', isRemoved : false})
                    if (existPartner) {
