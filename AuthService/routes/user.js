...
router.post("/information", async function (req, res, next) {
  if (req.body._id == 0) res.status(200).send("TOKEN INVALID");
  else {
    // tokrn valid
    const user = new User({});
    let existUser = await user.get({ _id: req.body._id, isRemoved: false });
    if (existUser.length == 0) {
      res.status(200).send("CE UTILISATEUR N'EXISTE PAS");
    } else {
      res.status(200).send({
        _id: req.body._id,
...
router.post("/updateUser", async function (req, res, next) {
  if (req.body._id == 0) res.status(200).send("TOKEN INVALID");
    ...
    if (!oldPassword)
      res.status(200).send("L'ANCIEN MOT DE PASSE EST OBLIGATOIRE");
    else {
      ...
      if (existUser.length == 0) {
        ...
        if (!match) {
          res.status(200).send("L'ANCIEN MOT DE PASSE EST INCORRECT");
        } else {
          //email
          if (req.body.email != existUser[0].email) {
            ...
            if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(req.body.email)) {
              isValid = false;
              res.status(200).send("L'ADRESSE MAIL N'EST PAS VALIDE");
            } else {
              ...
              if (userNewMail.length != 0) {
                isValid = false;
                res.status(200).send("CE UTILISATEUR EXISTE DEJA");
              } else {
          ...
          if (isValid) {
            ...
            if (!result.isValid) {
              isValid = false;
              res.status(200).send("LE NUMERO DE TELEPHONE N'EST PAS VALIDE");
