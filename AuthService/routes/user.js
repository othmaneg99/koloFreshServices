...
router.use(async function (req, res, next) {
    ...
    if(!token){req.body._id = 0}
    else{
        let infoUser = await verifyToken(token);
        if(!infoUser)req.body._id = 0 // token invalid
        else{
            let user = new User({});
...
router.post('/updateUser', async function(req,res,next){
    if(req.body._id == 0) res.status(401).send("TOKEN INVALID")
        ...
        if(!oldPassword) res.status(401).send("L'ANCIEN MOT DE PASSE EST OBLIGATOIRE")
            ...
            if(existUser.length == 0){
                res.status(401).send("CE UTILISATEUR N'EXISTE PAS")
            }else if(!existUser[0].password && existUser[0].isActive==true) res.status(401).send("VOUS AVEZ PAS LE DROIT DE CHANGER CES DONNEES (AUTHENTIFICATION PAR GOOGLE)")
            else{
                ...
                if(!match){
                    ...
                    if(isValid){ // email valid and exist
                        //phone
                        ...
                        if(isValid){ // phone valid and exist
