...
async function sendEmailNewPartner(nomPartner, phonePartner, nomRecipient, recipient) {
  try {
    const result = await mailjet
      .post("send", { version: "v3.1" })
      .request({
...
async function sendEmailDemande(nomPartner, phonePartner, nomRecipient, recipient, subject,demande) {
  try {
    const result = await mailjet
      .post("send", { version: "v3.1" })
      .request({
...
async function sendEmailResDemande(nomRecipient, recipient, subject,decision, demande) {
  try {
    const result = await mailjet
      .post("send", { version: "v3.1" })
      .request({
...
router.post('/newPassword', async function(req, res, next) {  
  let result = await sendEmailNewPassword(req.body.email, req.body.nom,req.body.password);
  res.send(result);
});

router.post('/newNameShop', async function(req, res, next) { 
  let result = await sendEmailDemande(req.body.nom, req.body.phone,req.body.nomAdmin, req.body.emailAdmin,"Demande de changement du nom du shop","le changement de son nom de shop, le nouveau nom est "+req.body.newNom);
  res.send(result);
});

router.post('/newCatShop', async function(req, res, next) { 
  let result = await sendEmailDemande(req.body.nom, req.body.phone,req.body.nomAdmin, req.body.emailAdmin,"Demande d'ajout d'une nouvelle catégorie","l'ajout d'une nouvelle catégorie: "+req.body.newCategorie);
  res.send(result);
});
...
router.post('/resNewNameShop', async function(req, res, next) {
  ...
  let result = await sendEmailResDemande(req.body.nom, req.body.email,"Réponse sur la demande de changement du nom du shop",decision,"de changement de votre nom du shop");
  res.send(result);
});

router.post('/resNewCatShop', async function(req, res, next) { 
  let decision = req.body.decision?"a accepté" : "a refusé"
  let result = await sendEmailResDemande(req.body.nom, req.body.email,"Réponse sur la demande d'ajout d'une nouvelle catégorie",decision,"d'ajout d'une nouvelle catégorie");
  res.send(result);
});
